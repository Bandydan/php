...
**Composer** — это паке№тный менеджер уровня приложений для языка программирования PHP, который предоставляет средства по управлению зависимостями в PHP-приложении.

В мире PHP существует достаточно много готовых решений. 
Например для работы с кешом на мне обязательно писать свою реализацию, мы можем взять готовую библиотеку или пакет.
...
В рамках данного занятия нам нужно рассмотреть только возможность автозагрузки в нашем простом пропекте MVC
...
- **name** название проекта через / по правилу Namespace/ProjectName. Имя должно быть уникальный по нему composer ищет пакеты [Link](https://getcomposer.org/doc/04-schema.md#name)
...
Данная комманда генерирует нам файл composer.loc в котором содержатся древо зависимостей
